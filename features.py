...
def get_market_features(target_date, ticker, recent_days=10):
    ...
    if len(vol) < 2:
        raise ValueError(f"Not enough {ticker} volume data (only {len(vol)} rows) up to {target_date}")
    logv = np.log(vol + 1)

    # yesterday’s log(volume+1)
    lag_vol = logv.shift(1).iloc[-1]

    # 5-day rolling std
    if len(logv) < 5:
        raise ValueError(f"Need ≥5 days of {ticker} data for rolling_std up to {target_date}")
